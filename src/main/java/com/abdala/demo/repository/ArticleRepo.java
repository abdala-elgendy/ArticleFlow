...
public interface ArticleRepo extends JpaRepository<Article, Long> {
    ...
    @Query("select ac from Article a left join ArticleComment ac " + "on a.id= :articleId")
    List<ArticleComment> findCommentsOfArticle(Long articleId);

    @Query("select ac from  Article ac " + "where ac.slug= :slug")
    ...
    @Query("SELECT  ac FROM Article ac WHERE LOWER(ac.content)" + " LIKE LOWER(CONCAT('%', :keyword, '%'))")
