package com.abdala.demo.config;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)
                .authorizeHttpRequests(req -> req.requestMatchers(WHITE_LIST_URL).permitAll()
                        .requestMatchers("api/auth/**").permitAll().requestMatchers("api/articles/**").permitAll()
                        .requestMatchers("api/favorites/**").permitAll().requestMatchers("api/users/**").permitAll()
                        // .requestMatchers(GET, "/api/users/{id}").hasAnyAuthority(ADMIN_READ.name())
                        // .requestMatchers(DELETE,
                        // "/api/articles/{id}").hasAnyAuthority(ADMIN_UPDATE.name(),ADMIN_DELETE.name())
